...
export const QnA = () => {
  ...
  // State to manage the sorting filter
  const [filter, setFilter] = useState('oldToNew');
  // State to manage the currently selected tag for filtering
  const [selectedTag, setSelectedTag] = useState<string | null>('All');
  // Function to handle tag selection for filtering
  ...
  // Function to handle clicking on the comments button
  const handleCommentClick = (index: number) => {
    alert(`Comments clicked for: ${comments[index].title}`);
  };
  // Filter and sort the comments based on selected filters
  ...
  return (
    <Page sx={{ height: '100%', overflowY: 'auto', padding: '20px' }}>
      ...
      {/* Dropdown to select sorting filter */}
